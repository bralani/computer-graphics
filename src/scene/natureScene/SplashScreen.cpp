...
namespace SplashScreen
{
  ...
  AppState run()
  {
    ...
    while (nextState == AppState::SPLASH_SCREEN && !glfwWindowShouldClose(window))
    {
      glfwPollEvents();
      ...
      ImGui::SetNextWindowSize(ImVec2(300, 220)); // Increased window size to fit larger buttons
      ImGui::Begin("Splash", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBackground);

      // Push the large font to use for the buttons
      ImGui::PushFont(largeFont);

      // Center the button and make it larger. New width is 200, so offset is 100.
      ImGui::SetCursorPosX(ImGui::GetWindowWidth() / 2.0f - 100);
      if (ImGui::Button("Play", ImVec2(200, 80))) // Increased button size
      {
      ...
      ImGui::SetCursorPosX(ImGui::GetWindowWidth() / 2.0f - 100);
      if (ImGui::Button("Exit", ImVec2(200, 80))) // Increased button size
      {
        nextState = AppState::EXIT;
      }

      // Pop the font to return to the default for any other UI elements
      ImGui::PopFont();
